...
- Always create the agent file first, then update all registration points
...
- **Context docs** — found by searching `.kiro/context/` for relevance.
...
After generating the agent file, update all registration points in the project.

### Registration Points

After creating the agent file, update all registration points in the project. Look for these common locations:
...
**2. Update the project constitution (`.kiro/steering/constitution.md` or equivalent):**
...
"{agent-id}": {
    "name": "{Display Name}",
    "description": "{1-line summary matching agent frontmatter description}",
...
- [ ] **Agent file exists** at `.kiro/agents/{agent-id}.md`
...
- [ ] **Agent-id consistent**: Same kebab-case ID across agent frontmatter, constitution.md tables, server.py dict
