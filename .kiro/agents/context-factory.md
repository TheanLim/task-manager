...
description: Context documentation specialist. Creates .kiro/context/ files as system blueprints with real code references, architecture patterns, and industry knowledge. Handles registration in MCP server subsystems and bidirectional cross-referencing.
...
- **Related context docs**: Found by searching existing `.kiro/context/` files
- **Content type**: Inferred from the domain (system doc, content definition, network protocol, visual spec, or blueprint)
- **Subsystem registration**: Which SUBSYSTEMS entries in `mcp-server/server.py` should reference this doc
- **.kiro/steering/constitution.md updates**: Only needed if this is a genuinely new subsystem not yet documented
...
### 3. Update constitution.md (only if needed)

Most context docs don't require .kiro/steering/constitution.md changes. Only update if:
