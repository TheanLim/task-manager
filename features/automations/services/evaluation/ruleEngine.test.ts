...
// ============================================================================
// Regression: create_card with interval/cron trigger and zero tasks
// This test catches the bug where create_card produced zero actions when
// there were no existing tasks, because the engine iterated allTasks.
// ============================================================================

describe('create_card with scheduled interval trigger', () => {
  it('produces exactly one action even when allTasks is empty', () => {
    const rule: any = {
      id: 'rule-create',
      projectId: 'proj-1',
      name: 'Daily standup card',
      trigger: {
    ...
    const context: EvaluationContext = {
      allTasks: [], // zero tasks — this was the bug
      allSections: [],
      maxDepth: 5,
      executedSet: new Set(),
    };
    ...
    // create_card should produce exactly one action regardless of task count
    expect(actions).toHaveLength(1);
    expect(actions[0].actionType).toBe('create_card');
    expect(actions[0].ruleId).toBe('rule-create');
  });

  it('produces exactly one action even when allTasks has many tasks', () => {
    const rule: any = {
      id: 'rule-create-2',
      projectId: 'proj-1',
    ...
    const event: DomainEvent = {
      ...
      entityId: rule.id,
      projectId: rule.projectId,
      changes: { triggerType: 'scheduled_cron' },
      previousValues: {},
      triggeredByRule: rule.id,
      depth: 0,
    };
    ...
    // create_card should produce exactly ONE action, not one per task
    expect(actions).toHaveLength(1);
    expect(actions[0].actionType).toBe('create_card');
  });

  it('non-create_card actions still produce one action per matching task', () => {
    const rule: any = {
      id: 'rule-complete',
      projectId: 'proj-1',
    ...
    // mark_card_complete should still produce one action per task
    expect(actions).toHaveLength(3);
    expect(actions.every(a => a.actionType === 'mark_card_complete')).toBe(true);
  });
});

describe('scheduled rule project scoping', () => {
  it('interval/cron rules only affect tasks in the same project', () => {
    const rule: any = {
      id: 'rule-proj-a',
      projectId: 'proj-a',
      name: 'Move overdue in proj-a',
      trigger: {
        type: 'scheduled_interval',
        sectionId: null,
        schedule: { kind: 'interval', intervalMinutes: 30 },
    ...
    const tasks = [
      { id: 'task-a1', projectId: 'proj-a', parentTaskId: null, sectionId: 'proj-a-todo', description: 'A1', completed: false },
      { id: 'task-a2', projectId: 'proj-a', parentTaskId: null, sectionId: 'proj-a-todo', description: 'A2', completed: false },
      { id: 'task-b1', projectId: 'proj-b', parentTaskId: null, sectionId: 'proj-b-todo', description: 'B1', completed: false },
      { id: 'task-b2', projectId: 'proj-b', parentTaskId: null, sectionId: 'proj-b-todo', description: 'B2', completed: false },
    ];
    ...
    // Should only produce actions for proj-a tasks, NOT proj-b tasks
    expect(actions).toHaveLength(2);
    expect(actions.every(a => a.targetEntityId.startsWith('task-a'))).toBe(true);
  });
});
