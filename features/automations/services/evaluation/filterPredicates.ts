...
// ============================================================================
// Age-based filter predicates (Phase 5b)
// ============================================================================

const DAY_MS = 86_400_000;

/**
 * Compute elapsed age in the appropriate unit.
 * For 'days': simple millisecond comparison (now - timestamp > value * DAY_MS).
 * For 'working_days': count working days between timestamp and now, compare > value.
 */
function ageExceedsThreshold(
  timestampMs: number,
  nowMs: number,
  value: number,
  unit: 'days' | 'working_days'
): boolean {
  if (unit === 'working_days') {
    const workingDays = countWorkingDaysBetween(new Date(timestampMs), new Date(nowMs));
    return workingDays > value;
  }
  return nowMs - timestampMs > value * DAY_MS;
}

filterPredicateMap.created_more_than = (task, filter, ctx) => {
  if (filter.type !== 'created_more_than') return false;
  const nowMs = ctx.now.getTime();
  const createdMs = new Date(task.createdAt).getTime();
  return ageExceedsThreshold(createdMs, nowMs, filter.value, filter.unit);
};

filterPredicateMap.completed_more_than = (task, filter, ctx) => {
  if (filter.type !== 'completed_more_than') return false;
  if (!task.completed) return false;
  // Legacy data: completed task with null completedAt → matches any threshold
  if (task.completedAt === null) return true;
  const nowMs = ctx.now.getTime();
  const completedMs = new Date(task.completedAt).getTime();
  return ageExceedsThreshold(completedMs, nowMs, filter.value, filter.unit);
};

filterPredicateMap.last_updated_more_than = (task, filter, ctx) => {
  if (filter.type !== 'last_updated_more_than') return false;
  const nowMs = ctx.now.getTime();
  const updatedMs = new Date(task.updatedAt).getTime();
  return ageExceedsThreshold(updatedMs, nowMs, filter.value, filter.unit);
};

filterPredicateMap.not_modified_in = (task, filter, ctx) => {
  if (filter.type !== 'not_modified_in') return false;
  const nowMs = ctx.now.getTime();
  const updatedMs = new Date(task.updatedAt).getTime();
  return ageExceedsThreshold(updatedMs, nowMs, filter.value, filter.unit);
};

filterPredicateMap.overdue_by_more_than = (task, filter, ctx) => {
  if (filter.type !== 'overdue_by_more_than') return false;
  if (task.dueDate === null) return false;
  if (task.completed) return false;
  const nowMs = ctx.now.getTime();
  const dueMs = new Date(task.dueDate).getTime();
  return ageExceedsThreshold(dueMs, nowMs, filter.value, filter.unit);
};

filterPredicateMap.in_section_for_more_than = (task, filter, ctx) => {
  if (filter.type !== 'in_section_for_more_than') return false;
  const nowMs = ctx.now.getTime();
  const effectiveTimestamp = task.movedToSectionAt ?? task.createdAt;
  const effectiveMs = new Date(effectiveTimestamp).getTime();
  return ageExceedsThreshold(effectiveMs, nowMs, filter.value, filter.unit);
};
