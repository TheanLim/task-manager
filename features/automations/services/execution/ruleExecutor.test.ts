...
// ── Bug fixes: execution log for scheduled create_card rules ────────────

describe('create_card execution log uses interpolated title (not raw template)', () => {
  let taskRepo: MockTaskRepository;
  let sectionRepo: MockSectionRepository;
  let ruleRepo: MockAutomationRuleRepository;
  let taskService: MockTaskService;
  ...
  function createTestSection(id = 'section-1', name = 'To Do'): Section {
    return {
      id, projectId: 'proj-1', name, order: 0, collapsed: false,
      createdAt: new Date().toISOString(), updatedAt: new Date().toISOString(),
    };
  }

  it('logs the interpolated title, not the raw {{day}} template', () => {
    const fakeClock = { now: () => new Date('2026-02-19T10:00:00Z').getTime(), toDate: () => new Date('2026-02-19T10:00:00Z') };
    executor = new RuleExecutor(taskRepo as any, sectionRepo as any, taskService as any, ruleRepo, fakeClock);
    ...
    const rule: AutomationRule = {
      id: 'rule-template',
      projectId: 'proj-1',
      name: 'Daily card with template',
      trigger: { type: 'scheduled_interval', sectionId: null, schedule: { kind: 'interval', intervalMinutes: 60 }, lastEvaluatedAt: null, catchUpPolicy: 'catch_up_latest' } as any,
      filters: [],
      action: {
        type: 'create_card',
        sectionId: 'section-1',
    ...
    const action: RuleAction = {
      ...
      params: { sectionId: 'section-1', position: 'top', cardTitle: 'Daily {{day}}', cardDateOption: undefined },
    };

    executor.executeActions([action], event);

    const updatedRule = ruleRepo.findById(rule.id)!;
    expect(updatedRule.recentExecutions.length).toBe(1);
    // Should contain the interpolated date, NOT the raw template
    expect(updatedRule.recentExecutions[0].taskName).toBe('Daily 2026-02-19');
    expect(updatedRule.recentExecutions[0].taskName).not.toContain('{{');
  });
});

describe('scheduled rule execution log has matchCount, details, and executionType', () => {
  let taskRepo: MockTaskRepository;
  let sectionRepo: MockSectionRepository;
  let ruleRepo: MockAutomationRuleRepository;
  let taskService: MockTaskService;
  ...
  it('sets matchCount, details, and executionType for scheduled rule actions', () => {
    const section: Section = {
      id: 'section-done', projectId: 'proj-1', name: 'Done', order: 0, collapsed: false,
      createdAt: new Date().toISOString(), updatedAt: new Date().toISOString(),
    };
    sectionRepo.create(section);

    const task1: Task = {
      id: 'task-1', projectId: 'proj-1', parentTaskId: null, sectionId: 'section-1',
      description: 'Task Alpha', notes: '', assignee: '', priority: 'none', tags: [],
      dueDate: null, completed: false, completedAt: null, order: 0,
      createdAt: new Date().toISOString(), updatedAt: new Date().toISOString(), lastActionAt: null,
    };
    const task2: Task = { ...task1, id: 'task-2', description: 'Task Beta', order: 1 };
    taskRepo.create(task1 as any);
    taskRepo.create(task2 as any);

    const rule: AutomationRule = {
      id: 'rule-sched', projectId: 'proj-1', name: 'Scheduled move',
      trigger: { type: 'scheduled_interval', sectionId: null, schedule: { kind: 'interval', intervalMinutes: 30 }, lastEvaluatedAt: null, catchUpPolicy: 'catch_up_latest' } as any,
      filters: [],
      action: { type: 'move_card_to_bottom_of_section', sectionId: 'section-done', dateOption: null, position: 'bottom', cardTitle: null, cardDateOption: null, specificMonth: null, specificDay: null, monthTarget: null },
      enabled: true, brokenReason: null, executionCount: 0, lastExecutedAt: null,
      recentExecutions: [], order: 0,
      createdAt: new Date().toISOString(), updatedAt: new Date().toISOString(),
    };
    ruleRepo.create(rule);

    const event: DomainEvent = {
      type: 'schedule.fired', entityId: rule.id, projectId: 'proj-1',
      changes: { triggerType: 'scheduled_interval' }, previousValues: {},
      triggeredByRule: rule.id, depth: 0,
    };

    // Two actions for the same scheduled rule
    const actions: RuleAction[] = [
      { ruleId: rule.id, actionType: 'move_card_to_bottom_of_section', targetEntityId: 'task-1', params: { sectionId: 'section-done', position: 'bottom' } },
      { ruleId: rule.id, actionType: 'move_card_to_bottom_of_section', targetEntityId: 'task-2', params: { sectionId: 'section-done', position: 'bottom' } },
    ];

    executor.executeActions(actions, event);

    const updatedRule = ruleRepo.findById(rule.id)!;
    // Should have ONE aggregated entry, not two individual entries
    expect(updatedRule.recentExecutions.length).toBe(1);

    const entry = updatedRule.recentExecutions[0];
    expect(entry.matchCount).toBe(2);
    expect(entry.details).toEqual(['Task Alpha', 'Task Beta']);
    expect(entry.executionType).toBe('scheduled');
  });
});
