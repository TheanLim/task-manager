...
describe('actionHandlers', () => {
  ...
  describe('create_card title template integration', () => {
    it('interpolates {{date}} placeholder using Clock', () => {
      // 2024-01-15 is a Monday
      const clock = new FakeClock(new Date('2024-01-15T12:00:00.000Z'));
      const ctxWithClock: ActionContext = { ...ctx, clock };
      sectionRepo._set('section-b', { id: 'section-b', name: 'Done', projectId: 'proj-1' });
      ...
      handler.execute(
        makeAction('create_card', {
          targetEntityId: 'irrelevant',
          params: { sectionId: 'section-b', cardTitle: 'Standup — {{date}}' },
        }),
        makeEvent(),
        ctxWithClock
      );
      ...
      const allTasks = taskRepo.findAll();
      expect(allTasks).toHaveLength(1);
      expect(allTasks[0].description).toBe('Standup — 2024-01-15');
    });

    it('interpolates {{weekday}} placeholder using Clock', () => {
      // 2024-01-15 is a Monday
      const clock = new FakeClock(new Date('2024-01-15T12:00:00.000Z'));
      const ctxWithClock: ActionContext = { ...ctx, clock };
      sectionRepo._set('section-b', { id: 'section-b', name: 'Done', projectId: 'proj-1' });
      ...
      handler.execute(
        makeAction('create_card', {
          targetEntityId: 'irrelevant',
          params: { sectionId: 'section-b', cardTitle: 'Weekly Review — {{weekday}}' },
        }),
        makeEvent(),
        ctxWithClock
      );
      ...
      const allTasks = taskRepo.findAll();
      expect(allTasks).toHaveLength(1);
      expect(allTasks[0].description).toBe('Weekly Review — Monday');
    });

    it('leaves title unchanged when no placeholders present', () => {
      const clock = new FakeClock(new Date('2024-01-15T12:00:00.000Z'));
      const ctxWithClock: ActionContext = { ...ctx, clock };
      sectionRepo._set('section-b', { id: 'section-b', name: 'Done', projectId: 'proj-1' });
      ...
      handler.execute(
        makeAction('create_card', {
          targetEntityId: 'irrelevant',
          params: { sectionId: 'section-b', cardTitle: 'Plain title no templates' },
        }),
        makeEvent(),
        ctxWithClock
      );
      ...
      const allTasks = taskRepo.findAll();
      expect(allTasks).toHaveLength(1);
      expect(allTasks[0].description).toBe('Plain title no templates');
    });

    it('supports templates for event-triggered create_card (not scheduled-only)', () => {
      // Event-triggered: no triggerType in changes, so dedup guard is skipped
      const clock = new FakeClock(new Date('2024-03-08T12:00:00.000Z'));
      const ctxWithClock: ActionContext = { ...ctx, clock };
      sectionRepo._set('section-b', { id: 'section-b', name: 'Done', projectId: 'proj-1' });
      ...
      handler.execute(
        makeAction('create_card', {
          targetEntityId: 'irrelevant',
          params: { sectionId: 'section-b', cardTitle: 'Review — {{date}} {{weekday}}' },
        }),
        makeEvent({ changes: {} }), // event-triggered, no triggerType
        ctxWithClock
      );

      const allTasks = taskRepo.findAll();
      expect(allTasks).toHaveLength(1);
      expect(allTasks[0].description).toBe('Review — 2024-03-08 Friday');
    });
  });
});
