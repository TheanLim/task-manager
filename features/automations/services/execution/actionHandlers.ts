...
import { calculateRelativeDate } from '../evaluation/dateCalculations';
import { shouldSkipCreateCard, getLookbackMs } from './createCardDedup';
import { interpolateTitle } from './titleTemplateEngine';
import { TRIGGER_SECTION_SENTINEL } from '../preview/rulePreviewService';
...
const createCardHandler: ActionHandler = {
  execute(action, triggeringEvent, ctx) {
    ...
    // Interpolate title templates ({{date}}, {{weekday}}, {{month}}) when Clock is available
    const resolvedTitle = ctx.clock ? interpolateTitle(cardTitle, ctx.clock) : cardTitle;
    ...
    if (triggerType && triggerType.startsWith('scheduled_')) {
      ...
      if (shouldSkipCreateCard(resolvedTitle, targetSectionId, allTasks, lookbackMs)) {
        return [];
      }
    }
