...
import { FILTER_META } from '../../services/preview/ruleMetadata';
...
const FILTER_CATEGORIES = [
  ...
  {
    label: 'Age / Activity',
    filters: FILTER_META.map((m) => ({ type: m.type as CardFilterType, label: m.label })),
  },
];
...
export function RuleDialogStepFilters({
  ...
  const handleAddFilter = (filterType: CardFilterType) => {
    ...
    if (filterType === 'in_section' || filterType === 'not_in_section') {
      ...
      filterType === 'created_more_than' ||
      filterType === 'completed_more_than' ||
      filterType === 'last_updated_more_than' ||
      filterType === 'not_modified_in' ||
      filterType === 'overdue_by_more_than' ||
      filterType === 'in_section_for_more_than'
    ) {
      newFilter = {
        type: filterType,
        value: 1,
