...
describe('RuleDialogStepFilters', () => {
  ...
  /**
   * Phase 5b — New age-based and section duration filter types
   *
   * Validates: R1 AC 6, R2 AC 4, R3 AC 4, R4 AC 5, R5 AC 9
   */
  describe('Phase 5b — New Filter Types in Dropdown', () => {
    it('shows all 6 new filter types in the "+ Add filter" dropdown', async () => {
      const user = userEvent.setup();
      ...
      // Verify all 6 new filter labels from FILTER_META are present
    ...
    it('shows "Age / Activity" category label in dropdown', async () => {
      const user = userEvent.setup();
      ...
      expect(screen.getByText('Age / Activity')).toBeInTheDocument();
    });

    it('adds created_more_than filter with default value and unit when selected', async () => {
      const user = userEvent.setup();
      ...
      await user.click(screen.getByRole('button', { name: /add filter/i }));
      await user.click(screen.getByText('Created more than...'));

      expect(mockOnFiltersChange).toHaveBeenCalledWith([
        { type: 'created_more_than', value: 1, unit: 'days' },
      ]);
    });

    it('adds overdue_by_more_than filter with default value and unit when selected', async () => {
      const user = userEvent.setup();
      ...
      await user.click(screen.getByRole('button', { name: /add filter/i }));
      await user.click(screen.getByText('Overdue by more than...'));

      expect(mockOnFiltersChange).toHaveBeenCalledWith([
        { type: 'overdue_by_more_than', value: 1, unit: 'days' },
      ]);
    });

    it('adds in_section_for_more_than filter with default value and unit when selected', async () => {
      const user = userEvent.setup();
      ...
      await user.click(screen.getByRole('button', { name: /add filter/i }));
      await user.click(screen.getByText('In current section for more than...'));

      expect(mockOnFiltersChange).toHaveBeenCalledWith([
        { type: 'in_section_for_more_than', value: 1, unit: 'days' },
      ]);
    });
  });

  describe('Phase 5b — New Filter Rendering (value + unit inputs)', () => {
    it('renders value input and unit select for created_more_than filter', () => {
      const filters: CardFilter[] = [
        { type: 'created_more_than', value: 5, unit: 'days' },
      ];
    ...
    it('renders value input and unit select for overdue_by_more_than filter', () => {
      const filters: CardFilter[] = [
        { type: 'overdue_by_more_than', value: 3, unit: 'working_days' },
      ];
    ...
    it('renders value input and unit select for in_section_for_more_than filter', () => {
      const filters: CardFilter[] = [
        { type: 'in_section_for_more_than', value: 7, unit: 'days' },
      ];
